...
const paket = [
  {
    id: 1,
    nama: "Paket 1 Camera",
    deskripsi: "Live streaming 1 kamera",
    created_at: "now",
  },
  {
    id: 2,
    nama: "Paket 2 Camera",
    deskripsi: "Live streaming 2 kamera",
    created_at: "now",
  },
  {
    id: 3,
    nama: "Paket 3 Camera",
    deskripsi: "Live streaming 3 kamera",
    created_at: "now",
  },
  {
    id: 4,
    nama: "Paket 4 Camera",
    deskripsi: "Live streaming 4 kamera",
    created_at: "now",
  },
];
    ...
    return {
      ...
      async first() {
        ...
        if (s.includes("FROM sessions JOIN admins"))
          return (
            sessions
              .filter((x) => x.id === a[0])
              .map((x) => ({ ...x, username: "admin" }))[0] ?? null
          );
        if (s.includes("FROM admins WHERE id"))
          return { id: 1, username: "admin" };
        if (s.includes("FROM paket WHERE id"))
          return paket.find((r) => r.id === a[0]) ?? null;
      ...
      async run() {
        ...
        if (s.startsWith("UPDATE paket SET")) {
          Object.assign(
            paket.find((r) => r.id === a[2]),
            { nama: a[0], deskripsi: a[1] },
          );
          return {};
        }
        if (s.startsWith("DELETE FROM paket_baris")) {
          for (let i = baris.length - 1; i >= 0; i--)
            if (baris[i].paket_id === a[0]) baris.splice(i, 1);
          return {};
        }
        if (s.includes("rab") || s.includes("transaksi"))
          throw new Error(
            `sham: route paket tak boleh menyentuh dokumen: ${s}`,
          );
...
const call = (path, opts = {}) =>
  app.request(
    path,
    { ...opts, headers: { ...(opts.headers ?? {}), host: "localhost" } },
    { DB },
  );
...
// 2. Seed totals match the Excel (Y2 as-is, anomalies included).
{
  ...
  check("subtotal kategori paket 1", () => {
    ...
    must(
      p1.subtotal.LOGISTIK === 150000 && p1.subtotal.MISC === 150000,
      "LOG/MISC salah",
    );
...
// 4. Create paket with rows → total derived; unknown jenis → 400.
{
  ...
  const bad = await call(
    ...
    json({
      nama: "X",
      baris: [{ jenis: "sewa", nama: "X", qty: 1, harga_satuan: 1 }],
    }),
  );
...
// 5. PATCH rows wholesale (replace) + nama-only patch keeps rows.
{
  ...
  const keep = await call(
    "/api/paket/5",
    json({ nama: "Paket Custom v3" }, "PATCH"),
  );
  const keepBody = await keep.json();
  const miss = await call("/api/paket/99", json({ nama: "X" }, "PATCH"));
  check("PATCH ganti baris + nama saja", () => {
    must(
      repBody.total === 350000 &&
        repBody.baris.length === 1 &&
        repBody.baris[0].alat_id === 3,
      "replace gagal",
    );
    must(
      keepBody.total === 350000 && keepBody.baris.length === 1,
      "nama-only harus pertahankan baris",
    );
    must(miss.status === 404, `unknown: ${miss.status}`);
  });
}
...
// tapi rab_baris dokumen lama tak ikut berubah — snapshot milik sendiri.
{
  ...
  check("patch baris → dokumen lama utuh (snapshot)", () => {
    ...
    must(
      JSON.stringify(rabBaris) === JSON.stringify(before),
      "rab_baris ikut berubah — snapshot rusak",
    );
    must(
      rabBaris[0].harga_satuan === 100000,
      `snapshot lama ${rabBaris[0].harga_satuan}`,
    );
  });
}
...
// 7. No DELETE route by design (same spirit as B4).
{
  ...
  check("DELETE paket → 404 (tak ada route)", () =>
    must(del.status === 404, `status ${del.status}`),
  );
}
