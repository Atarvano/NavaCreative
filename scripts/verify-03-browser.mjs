...
// --- Desktop: pin geometry + horizontal travel, no neighbor overlap ---
{
  ...
  if (Math.abs(geo.sectionH - geo.vh) <= 1) ok(`desktop section exactly one viewport (${geo.sectionH}px vs vh ${geo.vh})`);
  ...
  if (geo.headBottom <= geo.vh + 1) ok(`desktop head fits inside viewport (bottom ${Math.round(geo.headBottom)}px)`);
  else fail(`desktop head overflows viewport (bottom ${Math.round(geo.headBottom)}px)`);
  if (geo.trackW > 1440) ok(`desktop track overflows viewport (${geo.trackW}px), pin has distance to travel`);
  else fail(`desktop track ${geo.trackW}px fits in viewport: no horizontal travel`);
  ...
  const mid = await page.evaluate(() => {
    ...
    return {
      ...
      servicesVisible: rect("#services").bottom > 2 && rect("#services").top < 2,
  ...
  if (mid.trackX < -10) ok(`desktop track travels horizontally (x ${Math.round(mid.trackX)}px)`);
  ...
  const afterPin = await page.evaluate(() => {
    ...
    return { workBottom: r.bottom, fixed: getComputedStyle(document.querySelector("#work")).position };
  });
  if (afterPin.workBottom <= 2) ok("desktop pin releases: work scrolled fully past");
  else fail(`desktop pin stuck (work bottom ${Math.round(afterPin.workBottom)}px)`);
  ...
  const rises = await page.evaluate(() =>
    [...document.querySelectorAll(".work-card")].map((c) =>
      new DOMMatrixReadOnly(getComputedStyle(c).transform).m42,
    ),
  );
  if (rises.every((y) => Math.abs(y) < 1)) ok(`desktop all ${rises.length} cards settled at y=0 (rise fired once each)`);
  else fail(`desktop card y offsets unsettled: ${rises.map(Math.round).join(",")}`);
  ...
  if (Math.abs(resized.h - resized.vh) <= 1) ok(`desktop pin geometry survives resize (${resized.h}px vs vh ${resized.vh})`);
  else fail(`pin geometry broke on resize (${resized.h}px vs vh ${resized.vh})`);

  // Navigate away and back: no stuck pin, no leaked trigger, no errors.
  await page.goto(`${base}/photo-video.html`, { waitUntil: "load", timeout: 30000 });
  await page.waitForTimeout(1500);
  await page.goto(`${base}/index.html`, { waitUntil: "load", timeout: 30000 });
  await page.waitForTimeout(4500);
  ...
  if (!back.pinnedAtRest && back.spacers === 1) ok("navigate away+back: single live pin, nothing stuck");
...
// --- Mobile: never pins, snap carousel scrolls natively, cards fade up ---
{
  ...
  if (mob.trackOverflowX === "auto" || mob.trackOverflowX === "scroll") ok(`mobile track scrolls natively (overflow-x ${mob.trackOverflowX})`);
  else fail(`mobile track overflow-x = ${mob.trackOverflowX}, want auto/scroll`);
  if (mob.snapType.includes("mandatory") && mob.snapType.includes("x")) ok(`mobile track snaps (${mob.snapType})`);
  else fail(`mobile snap-type = ${mob.snapType}`);
  if (mob.trackScrollable) ok("mobile track overflows: carousel has somewhere to go");
  else fail("mobile track fits viewport: nothing to swipe");
  if (mob.cards.some((c) => c.w > 390)) fail("mobile card wider than viewport");
  else ok(`mobile cards fit viewport (${Math.round(mob.cards[0].w)}px first card)`);
  ...
  if (swiped.left > 100) ok(`mobile carousel swipes natively (scrollLeft ${Math.round(swiped.left)})`);
